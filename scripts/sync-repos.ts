...
class VersionSourceError extends Error {
  constructor(repoId: string, sourcePath: string, cause: unknown) {
    super(`Failed to read/parse version source for "${repoId}" at ${sourcePath}`);
...
class VersionResolutionError extends Error {
  constructor(repoId: string, sourcePath: string, valuePath: string[]) {
    super(`No version found for "${repoId}" at ${sourcePath} → ${valuePath.join(".")}`);
    this.name = "VersionResolutionError";
  }
}

class GitSubtreeError extends Error {
  constructor(repoId: string, action: "add" | "pull", exitCode: number, stderr: string) {
    super(`git subtree ${action} failed for "${repoId}" (exit ${exitCode}):\n${stderr}`);
...
function runSubtree(
  ...
  const args = ["subtree", action, `--prefix=${prefix}`, repo.url, ref, "--squash"];

  console.log(`  $ git ${args.join(" ")}`);
  if (dryRun) return;

  const result = spawnSync("git", args, { cwd: root, stdio: ["inherit", "inherit", "pipe"] });
  if (result.status !== 0) {
    throw new GitSubtreeError(repo.id, action, result.status ?? 1, result.stderr?.toString() ?? "");
  }
}
...
if (filterById) {
  ...
  if (repos.length === 0) {
    throw new RepoSelectionError(filterById, allRepos.map((r) => r.id));
  }
}
...
for (const repo of repos) {
  ...
  console.log(`Syncing ${repo.id} @ ${ref} (subtree ${action}${dryRun ? ", dry run" : ""})`);
